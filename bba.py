...
def main():
    ...
    fig_cse_packages_hist = px.histogram(df_cse_packages, x='Package', title='Distribution of Packages for BBA Branch', color_discrete_sequence=['#2ca02c'])
    ...
    query_package_cgpa = """
        SELECT CGPA, Package
        FROM placement_data
        WHERE Branch = 'BBA' AND Placed = 'Yes' AND CGPA IS NOT NULL AND Package IS NOT NULL
    """
    ...
    fig_company_placement_bar_horizontal = px.bar(df_company_placement, y='Company_placed', x='Placement_Count', 
                                                   title='Company-wise Placement Count for BBA Department',
