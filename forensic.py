...
def main():
    ...
    fig_forensic_placed_unplaced_pie.update_traces(marker=dict(colors=['#1f77b4', '#ff7f0e']))
    st.plotly_chart(fig_forensic_placed_unplaced_pie)
    ...
    fig_forensic_packages_hist = px.histogram(df_forensic_packages, x='Package', title='Distribution of Packages for FORENSIC Department', color_discrete_sequence=['#2ca02c'])
    st.plotly_chart(fig_forensic_packages_hist)
    ...
    query = """
        ...
        """
    backlog_forensic_package_data = fetch_data(query)

    # Create DataFrame from fetched data
    df_backlog_forensic_package = pd.DataFrame(backlog_forensic_package_data, columns=['Name', 'Branch', 'Package'])

    # Visualize the data with a scatter plot
    fig_backlog_forensic_package = px.scatter(df_backlog_forensic_package, x='Name', y='Package', color='Branch',
    ...
    fig_backlog_forensic_package.update_layout(showlegend=True)
    st.plotly_chart(fig_backlog_forensic_package)
    ...
    # # Display DataFrame
    # st.subheader("Students in Forensic Department")
    # st.write(df)
